...
function Appointment() {
  ...
  const handleDeleteAppointment = (appId) => {
    // Make a request to delete the appointment from the database
    axios.delete(`http://localhost:8080/doctor-appointment/${appId}`)
      .then(response => {
        // Filter out the appointment to be deleted from the local state
        const updatedAppointments = appointments.filter((appointment) => appointment.AID !== appId);
        setAppointments(updatedAppointments);
